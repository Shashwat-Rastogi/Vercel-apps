...
style.innerHTML = `
  ...
  .combo-popup {
    ...
    color: #FFD700;
    font-weight: 900;
    text-shadow: 0 4px 20px rgba(0,0,0,0.5), 0 0 10px rgba(255,215,0,0.8);
    z-index: 9999;
    pointer-events: none;
...
function playSound(type, streak) {
  try {
    ...
    // Ignore audio errors so the game never freezes.
  }
}
...
function makeOptions(answer, distractors) {
  const cleanDistractors = [...new Set(distractors)]
    .filter(Boolean)
...
function parseDistractors(rawDistractors, answer) {
  let distractorsList = [];

  if (Array.isArray(rawDistractors)) {
    distractorsList = rawDistractors;
  } else if (typeof rawDistractors === "string") {
    try {
      distractorsList = JSON.parse(rawDistractors);
    } catch (e) {
      try {
        distractorsList = JSON.parse(rawDistractors.replace(/'/g, '"'));
      } catch (e2) {
        distractorsList = rawDistractors
          .replace(/[\[\]]/g, "")
          .split(",")
  ...
  distractorsList = [...new Set(distractorsList)]
    .filter(Boolean)
    .filter((item) => item !== answer);

  if (distractorsList.length < 3) {
    distractorsList = ["A", "B", "C", "D"].filter((item) => item !== answer);
  }
...
async function buildExam() {
  ...
  try {
    ...
    const allQuestions = Array.isArray(allDbQuestions) ? allDbQuestions : [];

    MODULES.forEach((module) => {
      const moduleQuestions = uniqueQuestionsByText(
        allQuestions.filter(
          (q) =>
            q.module === module.key &&
            q.text &&
      ...
      let freshQuestions = moduleQuestions.filter((q) => !seenTexts.has(q.text));

      if (freshQuestions.length < module.count) {
        console.warn(`Not enough fresh questions for ${module.key}. Adding recycled questions.`);

        const freshTextSet = new Set(freshQuestions.map((q) => q.text));
        const recycledQuestions = moduleQuestions.filter((q) => !freshTextSet.has(q.text));

        freshQuestions = [...freshQuestions, ...shuffle(recycledQuestions)];
      }

      const selectedQuestions = shuffle(freshQuestions).slice(0, module.count);

      if (selectedQuestions.length < module.count) {
        console.warn(
          `${module.name} only has ${selectedQuestions.length}/${module.count} usable unique questions.`
        );
      }

      selectedQuestions.forEach((q) => {
        const distractorsList = parseDistractors(q.distractors, q.answer);
    ...
    console.error("Exam build failed:", error);
    alert("Exam build failed. Please try again.");
    return false;
  }
  ...
  if (!state.questions.length) {
    alert("No questions found. Please check your Supabase questions table.");
    return false;
  }
...
async function startExam() {
  ...
  const examReady = await buildExam();

  if (btn) {
    btn.innerHTML = `<span>Start Practice</span><span aria-hidden="true">&gt;</span>`;
    btn.disabled = false;
  }
...
function finishModuleByTimeout() {
  ...
  if (end === undefined) return finishExam();

  while (state.index <= end && state.index < state.questions.length) {
    state.answers.push({
...
function chooseAnswer(button, selected) {
  ...
  if (correct && state.streak > 0 && state.streak % 3 === 0) {
  ...
  MODULES.forEach((module) => {
    ...
    card.innerHTML = `
      <div>
        ...
        <span>${answers.length}/${module.count} Q - ${module.minutes} minutes</span>
      </div>
      <strong>${correct}/${answers.length || module.count}</strong>
    `;
...
function reviewMistakes() {
  ...
  misses.forEach((miss) => {
    ...
    card.innerHTML = `
      <span>${miss.module} - ${miss.topic}</span>
      <p><strong>Q:</strong> ${miss.text}</p>
      <p><strong>Your answer:</strong> ${miss.selected}</p>
      <p><strong>Correct:</strong> ${miss.answer}</p>
    `;
...
function renderHistory() {
  ...
  const totalQuestions = MODULES.reduce((sum, module) => sum + module.count, 0);

  list.innerHTML = "";
  $("#resumeExam").classList.toggle("hidden", !user?.inProgress);

  if (!attempts.length) {
    $("#bestScore").textContent = `Best 0/${totalQuestions}`;
    list.innerHTML = `<p class="empty-history">No saved results yet.</p>`;
    return;
  }

  const best = attempts.reduce(
    (top, attempt) => (attempt.correct > top.correct ? attempt : top),
    attempts[0]
  );
  ...
  attempts.slice(0, 5).forEach((attempt) => {
    ...
    item.innerHTML = `
      <div>
        <strong>${date}</strong>
        <span>${attempt.accuracy}% accuracy - ${attempt.score} points</span>
      </div>
      <div class="history-score">${attempt.correct}/${attempt.total || totalQuestions}</div>
    `;
...
function renderQuestBoard() {
  ...
  if (!badges.length) {
    rack.innerHTML = `<span class="empty-history">Badges unlock later.</span>`;
    return;
  }
...
function hintText(topic) {
  return "Eliminate the clearly wrong options first, then compare the final two.";
}
...
function useFifty() {
  ...
  const wrongButtons = [...document.querySelectorAll(".option-button")]
    .filter((button) => button.dataset.value !== answer)
    .slice(0, 2);
...
function renderRewards(rewards) {
  ...
  $("#rewardPanel").innerHTML = `
    <div class="reward-card">
      <span>XP Earned</span>
      <strong>+${rewards.earnedXp}</strong>
    </div>
    <div class="reward-card">
...
function moduleSummary() {
  return MODULES.map((module) => {
    ...
    return {
      key: module.key,
      name: module.name,
      total: answers.length || module.count,
...
function saveFinishedAttempt(correct, accuracy) {
  ...
  updateUser((user) => {
    ...
    if (moduleSummary().some((module) => module.correct === module.total && module.total > 0)) {
      award("Module Master");
    }
